...
model IceT01 "Simple  map-based ice model with connector"
  ...
  Modelica.Blocks.Interfaces.RealInput nTauRef "normalized torque request" annotation (
    Placement(transformation(extent = {{-20, -20}, {20, 20}}, rotation = 90, origin={-60,-80}),     iconTransformation(extent = {{-20, -20}, {20, 20}}, rotation = 90, origin={-60,-80})));
equation
  connect(toG_perHour.y, fuelCons) annotation (
    Line(points={{30,-57},{30,-64},{60,-64},{60,-80}},                                           color = {0, 0, 127}, smooth = Smooth.None));
  ...
  connect(limiter.u, nTauRef)
    annotation (Line(points={{-60,-28},{-60,-80}}, color={0,0,127}));
  annotation (
