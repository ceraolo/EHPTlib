...
package ECUs
  ...
  equation
    connect(powFilt.y, powToWref.u) annotation (Line(points={{26,39},{26,20},{-40,
            20},{-40,-58},{-10,-58}}, color={0,0,127}));
    connect(powFilt.y, conn.icePowRef) annotation (Line(points={{26,39},{26,20},
            {42,20},{42,72},{0,72},{0,80}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-3,-6},{-3,-6}},
  ...
  equation
    ...
    connect(socRef_.y, fbSOC.u1)
      annotation (Line(points={{-62,5},{-62,24},{-56,24}}, color={0,0,127}));
    connect(fbSOC.y, socErrToPow.u)
      annotation (Line(points={{-39,24},{-30,24}}, color={0,0,127}));
    connect(socErrToPow.y, add.u2)
      annotation (Line(points={{-7,24},{14,24},{14,16}}, color={0,0,127}));
    connect(add.y, powToWref.u) annotation (Line(points={{20,-7},{20,-20},{-26,-20},
            {-26,-58},{-10,-58}}, color={0,0,127}));
    connect(powFilt.y, add.u1)
      annotation (Line(points={{26,39},{26,16}}, color={0,0,127}));
    connect(add.y, conn.icePowRef) annotation (Line(points={{20,-7},{20,-20},{
            42,-20},{42,74},{0,74},{0,80}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-3,-6},{-3,-6}},
        horizontalAlignment=TextAlignment.Right));
    connect(fbSOC.u2, conn.batSOC) annotation (Line(points={{-48,32},{-48,80},{
            0,80}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-3,6},{-3,6}},
        horizontalAlignment=TextAlignment.Right));
    annotation (Icon(graphics={                                             Text(extent={{-100,88},
                {100,58}},
            textString="PSD-ecu2",
            textColor={0,0,0})}));
  end PsdEcu2;
  ...
  model PsdEcu3
    ...
    Modelica.Blocks.Sources.RealExpression realExpression(y=add.y)
      annotation (Placement(transformation(extent={{-68,26},{-48,46}})));
    Modelica.Blocks.Sources.RealExpression realExpression1(y=switch.y)
      annotation (Placement(transformation(extent={{-44,-68},{-24,-48}})));
  equation
    connect(hysteresis.y, conn.iceON) annotation (Line(points={{-34,65},{-34,80},{
            0,80}}, color={255,0,255}), Text(
        string="%second",
        index=1,
        extent={{-3,6},{-3,6}},
        horizontalAlignment=TextAlignment.Right));
    connect(switch.y, conn.icePowRef) annotation (Line(points={{17,22},{42,22},{42,
            80},{0,80}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(hysteresis.y, switch.u2) annotation (Line(points={{-34,65},{-34,72},{-16,
            72},{-16,22},{-6,22}}, color={255,0,255}));
    connect(powFilt.y, add.u1)
      annotation (Line(points={{26,39},{26,-8}}, color={0,0,127}));
    connect(socErrToPow.y, add.u2) annotation (Line(points={{-31,-12},{-8,-12},{-8,
            -8},{14,-8}}, color={0,0,127}));
    connect(fbSOC.y, socErrToPow.u)
      annotation (Line(points={{-63,-12},{-54,-12}}, color={0,0,127}));
    connect(fbSOC.u1, socRef_.y)
      annotation (Line(points={{-80,-12},{-86,-12},{-86,-31}}, color={0,0,127}));
    connect(fbSOC.u2, conn.batSOC) annotation (Line(points={{-72,-4},{-72,80},{0,80}},
          color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-3,6},{-3,6}},
        horizontalAlignment=TextAlignment.Right));
    connect(constZero.y, switch.u3)
      annotation (Line(points={{-21.4,14},{-6,14}}, color={0,0,127}));
    connect(realExpression.y, hysteresis.u)
      annotation (Line(points={{-47,36},{-34,36},{-34,42}}, color={0,0,127}));
    connect(realExpression1.y, powToWref.u)
      annotation (Line(points={{-23,-58},{-10,-58}}, color={0,0,127}));
    connect(hysteresis.u, switch.u1)
      annotation (Line(points={{-34,42},{-34,30},{-6,30}}, color={0,0,127}));
    annotation (Icon(graphics={                                             Text(extent={{-100,86},
                {100,56}},
            textString="PSD-ecu3",
            textColor={0,0,0})}));
...
<p>- further ON/OFF control to switch OFF the engine when the average power is too low to permit efficient operation</p>
  ...
  equation
    ...
    connect(maxTau.u[1], toPuSpeed.y)
      annotation (Line(points={{-14,78},{-27,78},{-27,38.7}}, color={0,0,127}));
    connect(toPuSpeed.u, Wmecc) annotation (Line(points={{-27,22.6},{-27,12},{0,12},
            {0,-98}},                  color={0,0,127}));
    ...
    connect(feedback.u1, optiSpeed.y[1])
      annotation (Line(points={{26,-30},{-59,-30}}, color={0,0,127}));
    annotation (
...
<p>The control logic commands the genset to deliver at the DC port the input power, using the optimal generator speed.</p>
  ...
  package Partial
    ...
    equation
      ...
      connect(maxTau.u[1], toPuSpeed.y)
        annotation (Line(points={{-14,78},{-27,78},{-27,38.7}}, color={0,0,127}));
      connect(toPuSpeed.u, Wmecc) annotation (Line(points={{-27,22.6},{-27,12},
              {0,12},{0,-98}},           color={0,0,127}));
      connect(gain1.u, fromPuTorque.y) annotation (Line(points={{52,30.4},{52,26},
              {34,26},{34,77},{32.7,77}}, color={0,0,127}));
      connect(division.u2, Wmecc) annotation (Line(points={{-56,32},{-56,0},{0,
              0},{0,-98}}, color={0,0,127}));
...
<p>The control logic commands the genset to deliver at the DC port the input power, using the optimal generator speed.</p>
    ...
    partial model PartialEcu
      ...
      SupportModels.ConnectorRelated.Conn conn annotation (Placement(
          visible=true,
      ...
      Modelica.Blocks.Tables.CombiTable1Ds powToWref(
        fileName="wToTau.txt",
        tableOnFile=false,
      ...
      Modelica.Blocks.Interfaces.RealInput tauRef annotation (
        Placement(visible = true, transformation(extent={{-158,-20},{-118,20}},      rotation = 0), iconTransformation(extent={{-140,
                -20},{-100,20}},                                                                                                                           rotation = 0)));
    equation
      connect(powFilt.u, conn.motPowDelB) annotation (Line(points={{26,62},{26,
              70},{0,70},{0,80}}, color={0,0,127}));
      connect(gain.u, feedback.y) annotation (
        Line(points={{76,-58},{69,-58}},      color = {0, 0, 127}));
      connect(feedback.u2, conn.iceW) annotation (Line(
          points={{60,-50},{60,76},{0,76},{0,80}},
          color={0,0,127},
          smooth=Smooth.None), Text(
          string="%second",
          index=1,
          extent={{6,3},{6,3}}));
      connect(limiter.y, conn.genTauRef) annotation (Line(
          points={{98,51},{98,78},{0,78},{0,80}},
          color={0,0,127},
          smooth=Smooth.None), Text(
          string="%second",
          index=1,
      ...
      connect(toNm.y, conn.motTauRef) annotation (Line(
          points={{-108,43},{-108,80},{0,80}},
          color={0,0,127},
          smooth=Smooth.None), Text(
          string="%second",
          index=1,
      ...
      connect(limiter1.u, powToWref.y[1]) annotation (Line(
          points={{20,-58},{13,-58}},
          color={0,0,127},
          smooth=Smooth.None));
      ...
      annotation (
        Diagram(coordinateSystem(extent={{-120,-80},{120,80}},      preserveAspectRatio = false, initialScale = 0.1), graphics={                                                                                      Text(extent={{28,62},
                  {68,58}},
              textColor={0,0,0},
              textString="send
PowRef
...
reference tau
to gen",
       horizontalAlignment = TextAlignment.Left), Line(origin={48,50},    points={{0, 6},{0,-6},{0,-6}},                                                                          arrow = {Arrow.Filled, Arrow.None}),
              Line(origin={64,52},    points = {{0, -6}, {0, 6}, {0, 6}}, arrow = {Arrow.Filled, Arrow.None}),
              Line(origin={94,66},    points={{0,6},{0,-6},{0,-6}},                                                                          arrow = {Arrow.Filled, Arrow.None}),
                                                                                                                                  Text(extent={{-106,74},
                  {-92,62}},                                                                                                                                            textString = "Send 
requested
torque 
to mot",
       horizontalAlignment = TextAlignment.Left)}),
        Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = false,
           ...
           fillPattern =  FillPattern.Solid, points = {{-68, 2}, {-48, -8}, {-56, -14}, {-76, -4}, {-68, 2}}), Polygon(lineColor = {95, 95, 95}, fillColor = {75, 75, 75},
           fillPattern =  FillPattern.Solid, points = {{-64, -8}, {-4, -40}, {-4, -62}, {-64, -30}, {-64, -8}}), Polygon(lineColor = {95, 95, 95}, fillColor = {160, 160, 164},
           fillPattern =  FillPattern.Solid, points = {{-64, -8}, {-4, -40}, {74, 16}, {14, 48}, {-64, -8}})}),
        Documentation(info="<html>
